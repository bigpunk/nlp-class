...
def make_emissions_dict(word_counts):
    ...
    for word,lcs in tracker.iteritems():
        for label,count in lcs.iteritems():
            ...
            key = "%s|%s" % (word, label)
            ret[key] = -math.log(count / label_counts[label])
    ...
    for i in labels:
        for i_minus_1 in labels:
            for i_minus_2 in labels:
                ...
                ret[q_key] = -math.log(three_grams[three_key] / two_grams[two_key])
                    ...
                    for w in k_two_tags:
                        ...
                        if e_key not in em_dict:
                            ...
                            if temp_e_key in em_dict:
                                em_dict[e_key] = 9999999999999999
                            else:
                                e_key = "%s|%s" % (determine_class(word), v)
...
if __name__ == "__main__":
    ...
    # filter lines without WORDTAGE so we get counts like:
